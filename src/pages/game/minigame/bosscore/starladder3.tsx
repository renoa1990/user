...
const Starladder3Page: NextPage<
...
> = ({ gameSetup, lvsetup, minigameSetup, data, userMoney, mutate }) => {
  ...
  useEffect(() => {
    ...
    const updateTimer = () => {
      ...
      const playTime = new Date(data.starladder3.Playtime);
  ...
  const odds: LadderOdds = {
    ldOddOdds: 0,
    ldEvenOdds: 0,
    ldLeftOdds: parseFloat(data?.starladder3?.game_odd_left || "1.95"),
    ldRightOdds: parseFloat(data?.starladder3?.game_odd_right || "1.95"),
    ld3LineOdds: parseFloat(data?.starladder3?.game_odd_line3 || "2.95"),
    ld4LineOdds: parseFloat(data?.starladder3?.game_odd_line4 || "2.95"),
    ldLeft3EvenOdds: 0,
    ldLeft4OddOdds: 0,
    ldRight3OddOdds: 0,
    ldLeft4EvenOdds: 0,
  ...
  const codeMapping: Record<string, any> | null = !data?.starladder3
    ? null
    : {
        LEFT: {
          gameType: "starladder_rl",
          pick: "좌",
          pickOdds: data.starladder3.game_odd_left,
          team_home: "좌",
          team_away: "우",
          Odds_home: data.starladder3.game_odd_left,
          Odds_away: data.starladder3.game_odd_right,
        },
        RIGHT: {
          gameType: "starladder_rl",
          pick: "우",
          pickOdds: data.starladder3.game_odd_right,
          team_home: "좌",
          team_away: "우",
          Odds_home: data.starladder3.game_odd_left,
          Odds_away: data.starladder3.game_odd_right,
        ...
        LINES3: {
          ...
          pick: "3줄",
          pickOdds: data.starladder3.game_odd_line3,
          team_home: "3줄",
          team_away: "4줄",
          Odds_home: data.starladder3.game_odd_line3,
          Odds_away: data.starladder3.game_odd_line4,
        },
        LINES4: {
          gameType: "starladder_line",
          pick: "4줄",
          pickOdds: data.starladder3.game_odd_line4,
          team_home: "3줄",
          team_away: "4줄",
          Odds_home: data.starladder3.game_odd_line3,
          Odds_away: data.starladder3.game_odd_line4,
  ...
  const handleSelect = (code: string | null) => {
    if (isDisable || !data?.starladder3 || !codeMapping) return;
    ...
    const cartData: cartState = {
      id: data.starladder3.id,
      gameCode: "미니게임",
      game_Event: "별다리 3분",
      game_Name: `${data.starladder3.round} 회차`,
      game_Type: gameData.gameType,
      pickOdds: gameData.pickOdds,
      pick: gameData.pick,
      game_Time: data.starladder3.Playtime,
  ...
  return (
    <>
      <Head>
        <title>별다리 3분 - 미니게임</title>
      </Head>
      <main>
        <MinigameLayoutNew
          list={data?.list}
          src="https://boscore.com/ladder/live3.php"
          title="별다리 3분"
          contentWidth={800}
          contentHeight={600}
          roundNo={data?.starladder3?.round}
          timeLeft={timeLeft}
          gameCode="미니게임"
          game_Event="별다리 3분"
          userMoney={userMoney}
        ...
        >
          <LadderPanel
            ...
            disabled={!data?.starladder3 || isDisable}
          />
        </MinigameLayoutNew>
      </main>
    </>
...
Starladder3Page.displayName = "Starladder3Page";

const Page: NextPage<serverSideProps> = (props) => {
  const { data, mutate } = useSWR<SWR>(`/api/minigame/timecounter/별다리 3분`, {
